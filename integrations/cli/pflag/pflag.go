...
//
// See github.com/fastabc/fastconf/providers/cliflag for the underlying design:
// this package is a thin layer that walks pflag's "changed" set and feeds
// each pair into cliflag.From, which performs the dotted-key nesting.
//
...
//
// Flag names containing "." nest into sub-maps (see cliflag docs).
...
func FromChanged(fs *flagpkg.FlagSet) map[string]any {
	return cliflag.From(func(yield func(name, value string)) {
