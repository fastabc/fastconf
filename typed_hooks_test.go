...
// BUG-1201 regression: YAML "30s" decoded into a time.Duration field
// silently dropped to 0 in v0.11.0 because encoding/json refuses the
// string→Duration conversion. Phase 130 typed hooks fix this; lock it
// in here so a future refactor doesn't regress the contract.
type bug1201Cfg struct{ Timeout time.Duration `json:"timeout"` }

func TestTypedHook_DurationFromYAMLString(t *testing.T) {
	fs := fstest.MapFS{
		"conf.d/base/00.yaml": &fstest.MapFile{Data: []byte("timeout: 250ms\n")},
	}
	mgr, err := fastconf.New[bug1201Cfg](context.Background(),
		fastconf.WithFS(fs),
		fastconf.WithDir("conf.d"),
	)
	if err != nil {
	...
	if got := mgr.Get().Timeout; got != 250*time.Millisecond {
		t.Errorf("BUG-1201 regression: Timeout = %v, want 250ms", got)
	}
}
