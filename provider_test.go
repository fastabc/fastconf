...
// ── Folded from provider_internal_test.go (Phase 84 SPEC-84) ──
...
type bug1206Cfg struct {
	Greeting string `json:"greeting"`
}

// BUG-1206: WithDotEnvAuto placed BEFORE WithDir used to silently fail
// because path resolution captured the empty o.dir at option time. Fix
// defers resolution to New(). Regression test exercises the worst-case
// option order.
func TestProvider_DotEnvAutoRegardlessOfOrder(t *testing.T) {
	...
	mgr, err := fastconf.New[bug1206Cfg](context.Background(),
	...
	if got := mgr.Get().Greeting; got != "hello-from-dotenv" {
		t.Errorf("Greeting = %q, want hello-from-dotenv (BUG-1206 regression)", got)
	}
}
