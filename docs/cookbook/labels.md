...
| 来自 Docker engine / K8s controller / CLI `--label` 等外部注入 | `WithProvider(provider.NewLabels(...))` / `WithProvider(provider.NewLabelMap(...))` | 走 provider 优先级，默认 `PriorityCLI` |
...
Provider 默认优先级是 `PriorityCLI`，即在 file 与 env 之上；可用 `LabelOptions.Priority` 调整。
