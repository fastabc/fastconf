...
5. [核心模型](#核心模型)
6. [核心抽象](#核心抽象)
7. [公开 API 地图](#公开-api-地图)
8. [Option 参考](#option-参考)
9. [Reload 流水线](#reload-流水线)
10. [Profile 与 Overlay](#profile-与-overlay)
11. [Provider 系统](#provider-系统)
12. [Transformer 与 Migration](#transformer-与-migration)
13. [Watch、Subscribe 与 Plan](#watchsubscribe-与-plan)
14. [Provenance、History 与 Rollback](#provenancehistory-与-rollback)
15. [可观测性](#可观测性)
16. [多租户与 Preset](#多租户与-preset)
17. [为什么没有 `GetString("a.b.c")`](#为什么没有-getstringabc)
18. [性能与可靠性](#性能与可靠性)
19. [Sub-module 生态矩阵](#sub-module-生态矩阵)
20. [扩展指南](#扩展指南)
21. [CLI 工具](#cli-工具)
22. [本地开发](#本地开发)
23. [文档地图](#文档地图)
...
|---|---|---|---|
| **spf13/viper** | `viper.BindPFlag(...)` | `provider.NewCLIChanged(cliflag.FromChanged(cmd.Flags()))` | `BindPFlag` 会把 pflag **默认值** 也注入配置 → 覆盖 YAML / env 的合法值；FastConf 只转发用户显式 set 过的 flag。 |
...
| **spf13/cobra + pflag** | `cmd.Flags()` | `cliflag.FromChanged(cmd.Flags())` → `provider.NewCLIChanged(...)` | sub-module `integrations/cli/pflag`，避免 pflag 进入根 module 依赖闭包。 |
| **stdlib `flag`** | `flag.FlagSet` | `cliadapter.FromStdFlag(fs)` → `provider.NewCLIChanged(...)` | 零依赖；在 `pkg/cliadapter`。 |
